...
fn main() {
    ...
    thread::spawn(move || {
        ...
        controller
            .lock()
            .unwrap()
            .register_device_event_callback(Arc::new(Mutex::new(
                move |event: SystemEvent| -> Result<(), Box<dyn Error + Send>> {
        ...
        controller
            ...
            .register_wheel_event_callback(Arc::new(Mutex::new(
                move |(wheel, pos): (Wheel, u8)| -> Result<(), Box<dyn Error + Send>> {
        ...
        controller
            .lock()
            .unwrap()
            .register_button_event_callback(Arc::new(Mutex::new(
                move |button: Button| -> Result<(), Box<dyn Error + Send>> {
